...
import traceback
from lcasr.utils.dataloading import Utterance_Dataloader, reset_seen_ids, MaskedUtterance_Dataset
...
def train(
    ...
    for epoch in range(epoch, max_epochs):
        ...
        for ix, batch in enumerate(dataloader):
            ...
            lm_text, lm_text_lengths = batch['lm_text'], batch['lm_text_lengths']
            lm_loss_mask = batch['lm_loss_mask']
            ...
            other_args = {}
            other_args['lm_text_sequence'] = lm_text.to(device)
            other_args['lm_text_sequence_lengths'] = lm_text_lengths.to(device)
            if lm_loss_mask is not None: other_args['lm_loss_mask'] = lm_loss_mask.to(device)
            other_args['wandb'] = wandb
            other_args['optimizer'] = optimizer

            out = model.calc_loss(
                audio_signal = audio, 
                text_sequence = text.to(device),
                a_lengths = audio_lengths.to(device),
                t_lengths = text_lengths.to(device),
                tokenizer = tokenizer,
                **other_args,
            )
            ...
            blank_prob = blank_p(cur_probs.detach(), tokenizer) if exists(cur_probs) else 0.0
            ...
            if not hasattr(model, 'calc_loss_includes_backward') or model.calc_loss_includes_backward == False:
                if (ix+1) % backwards_every == 0 or (ix+1) == len(dataloader):
                    (((backwards_every_loss) / counter['steps_since_backwards'])).backward() # divide by chunk*batch_size constant to weight smaller batches less
                    counter['steps_since_backwards'] = 0
                    backwards_every_loss = 0

            if (ix+1) % backprop_every == 0 or (ix+1) == len(dataloader):
...
def main(args):
    ...
    dataloader = Utterance_Dataloader(
        ...
        dataset = partial(MaskedUtterance_Dataset, tokenizer=tokenizer, zipf_path=args.config['data'].get('zipf_freq_path', None), masking=args.config['training'].get('mask_pass', True)),
    )

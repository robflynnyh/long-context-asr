        ...
        try:
            for ix, chunk_json in enumerate(chunks):
                ...
                with autocast(device.type, dtype=dtype) if torch.cuda.is_available() else nullcontext():
                    ...
                    second_pass = model(
                        downsampled_audio = first_pass['downsampled_audio'].detach(),
                        downsampled_audio_length = first_pass['length'].detach(),
                        prev_posteriors = first_pass['final_posteriors'].exp().detach(),
                    )

                    final_posteriors = torch.cat([first_pass['final_posteriors'], second_pass['final_posteriors']], dim=0)
                    length = torch.cat([first_pass['length'], second_pass['length']], dim=0)
                    txt = torch.cat([txt, txt], dim=0)
                    t_lengths = torch.cat([t_lengths, t_lengths], dim=0)
                    out = {'final_posteriors': final_posteriors, 'length': length}
                ...
                cur_tokens_in_loss += (sum(a_lengths))*2 # total number of acoustic frames in batch

                if (ix+1) % backwards_every == 0 or (ix+1) == len(chunks):
                    scaler.scale(((backwards_every_loss) / (chunk_size*batch_size*2)*steps_since_backwards) * 100).backward() # divide by chunk*batch_size constant to weight smaller batches less

...
def train(
    ...
    for epoch in range(epoch, max_epochs):
        ...
        for batch in dataloader:
            ...
            repr_grads = torch.autograd.grad(original_loss, model.reprs, create_graph=False, retain_graph=True)[0].to(model.grad_pred.dtype).detach()

            q, indice, _ = model.grad_vq(repr_grads)
        
            meta_grad_pred = model.grad_pred
            meta_grad_pred_1, meta_grad_pred_2 = meta_grad_pred, meta_grad_pred.detach()
            meta_loss_1 = meta_loss_fn(rearrange(indice, 'b n -> (b n)'), rearrange(meta_grad_pred_1, 'b n v -> (b n) v'), d=(batch_size*chunk_size))
            meta_loss_2 = meta_loss_fn(rearrange(indice[torch.randperm(repr_grads.shape[0])], 'b n -> (b n)'), rearrange(meta_grad_pred_2, 'b n v -> (b n) v'), d=(batch_size*chunk_size))
            
            print(meta_grad_pred_1.shape, model.grad_vq.codebook.shape)
            #pred = (meta_grad_pred_1.detach().softmax(-1).unsqueeze(-1) * model.grad_vq.codebook.unsqueeze(0).unsqueeze(0)).sum(-2)
            # above is too much memory do as an einsum
            indices = meta_grad_pred_1.detach().argmax(-1, keepdim=True)
            out = torch.zeros_like(meta_grad_pred_1).scatter_(-1, indices, 1.0)
            grads = einsum(out, model.grad_vq.codebook, 'b n c, c d -> b n d')

            cosim = (torch.nn.functional.cosine_similarity(repr_grads, grads, dim=-1) * -1 + 1).mean()
            ...
            bp_layers = [p for p in model.meta_layers.parameters()]
            bp_layers += [p for p in model.meta_decoder.parameters()]
            (meta_loss_1).backward(inputs=bp_layers)
...
def main(args):
    ...
    print(f'using model class: {get_model_class(config = args.config)}')

        ...
        try:
            for ix, chunk_json in enumerate(chunks):
                ...
                if (ix+1) % backwards_every == 0 or (ix+1) == len(chunks):
                    ...
                    for param in model.meta_proj.fn.parameters():
                        target_grad = (param.grad * inv_scale).detach()

                    meta_grad_pred = model.grad_preds
                    meta_grad_pred = meta_grad_pred.reshape(-1)
                    target_grad = target_grad.reshape(-1)
                    meta_grad_pred = rearrange(meta_grad_pred, '(a b) -> a b', b=24)
                    target_grad = rearrange(target_grad, '(a b) -> a b', b=24)
                    
                    meta_loss = (torch.nn.functional.cosine_similarity(target_grad, meta_grad_pred, dim=-1) * -1 + 1) * meta_loss_scale
                    meta_loss = meta_loss.sum()
                    ...
                    # meta_grad_pred = model.grad_preds
                    # # print(repr_grads.shape, repr_grad_pred.shape, '!')
                    # # #print(repr_grad_pred)
                    # if args.config['training'].get('loss', 'cosine') == 'cosine':
                    #     meta_loss = (torch.nn.functional.cosine_similarity(repr_grads, meta_grad_pred, dim=-1) * -1 + 1) * meta_loss_scale
               
                    #     meta_loss = meta_loss.mean()
                    # elif args.config['training'].get('loss', 'cosine') == 'l2':
                    #     meta_loss = ((((repr_grads - meta_grad_pred) ** 2).sum(-1)) ** 0.5).mean() * meta_loss_scale
                    # else:
                    #     raise ValueError('invalid loss')
                    
                    # repr_grad_norm = torch.norm(repr_grads)
                    
                    # #model.reprs.grad = None
                    # scaler.scale(meta_loss).backward()# + (-repr_grad_norm)).backward()
                    # #scaler.scale(meta_loss + (-model.reprs.norm())/50).backward()

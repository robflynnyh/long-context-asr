...
def train(
        ...
        augmentation_model:torch.nn.Module,
    ...
    # iterate all modules and find modules that are instances of:Attention
            ...
            for ix, chunk_json in enumerate(chunks):
                ...
                with autocast(device.type, dtype=dtype) if torch.cuda.is_available() else nullcontext():
                    ...
                    for k, v in grads.items():
                        batched_detached_asr_params[k] = batched_detached_asr_params[k] - v * augmentation_model.asr_model_lr 
                        
               
                    vmap_forward_fn = partial(vmap_forward, model=asr_model)
                    out = vmap(vmap_forward_fn)(batched_detached_asr_params, audio)
                    predictions = out['final_posteriors'].squeeze(1)
                    loss = ctc_loss_fn(predictions.transpose(0,1), txt, out['length'].squeeze(1), t_lengths).sum()
                    # out = model(
                    #     audio_signal = audio, 
                    #     length = a_lengths, 
                ...
                if (ix+1) % backprop_every == 0 or (ix+1) == len(chunks): 
                    ...
                    teacher_loss = ((teacher_loss / cur_tokens_in_loss) * 100).item()
                    loss_diff = teacher_loss - loss_to_log
                    print(f'loss: {loss_diff}')
                    ...
                    if wandb_config['use']:
                        wandb.log({
                            'loss': loss_diff,
                            'augmentation_model_lr': augmentation_model.asr_model_lr.item(),
...
def main(args):
    ...
    if wandb_config['use']:
        ...
        wandb.watch(augmentation_model, log="all") # sometimes this causes a crash ):
    ...
    optimizer, scheduler = load_optimizer(args.config, augmentation_model)

    ...
    while not finished:#################
        ...
        try:
            ...
            with autocast(device.type, dtype=dtype) if torch.cuda.is_available() else nullcontext():
                ...
                loss_pred, wer_pred = model.metric_preds.unbind(dim=-1)
                loss_loss = torch.nn.functional.mse_loss(loss_pred.squeeze(-1), original_loss / a_lengths, reduction='mean')
                
                wer = calc_wer(ctc_decoder, cur_probs, a_lengths, txt_chunks_untokenized)
             
                wer_loss = torch.nn.functional.mse_loss(wer_pred.squeeze(-1), wer.to(device), reduction='mean')
            ...
            all_loss = (main_loss + loss_loss + wer_loss) / 3

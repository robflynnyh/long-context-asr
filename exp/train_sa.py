...
import traceback
from lcasr.utils.dataloading import VariableBatchSimpleDataloader, chunk_spectogram, chunk_text_and_speakers_json, reset_seen_ids
...
def train(
    ...
    while not finished:#################
        ...
        speaker_and_txt_chunks = [chunk_text_and_speakers_json(text = el, chunk_size = chunk_size, chunk_overlap = chunk_overlap, spectogram_length = audio.shape[-1]) for el in txt] # becomes v slow for v large batch sizes !!
        
        txt_chunks, speaker_chunks = [],[]
        for el in speaker_and_txt_chunks: txt_chunks.append(el[0]); speaker_chunks.append(el[1])
        ...
        for ix, el in enumerate(audio_chunks_):
            ...
            enc_speaker_chunks = {i:f'speakers: {el[ix]}. ' for i, el in enumerate(speaker_chunks) if remove_mask[i]}
            enc_txt_chunks = [torch.LongTensor(tokenizer.encode(enc_speaker_chunks[i] + el[ix])) for i, el in enumerate(txt_chunks) if remove_mask[i]]
       
            assert len(enc_speaker_chunks) == len(enc_txt_chunks), 'speaker and text chunks must be same length'
...
def main(args):
    ...
    model = load_model(args.config, tokenizer.vocab_size(), model_class = get_model_class(config = {'model_class': args.config.get('model_class', 'SCConformerXL')}))

...
- ROB-27 smoke probe: created smoke-test config/script under `/mnt/parscratch/users/acp21rjf/symphony-job-artifacts/ROB-27/` and submitted job `10094359`; actual start estimate was `2026-05-05T08:30:00`, later than full job `10094264`, so `10094359` was cancelled before it ran.
- ROB-27 queue probe: 80GB 48h H100-NVL test-only submission estimated `2026-05-12T23:50:50`, later than existing full job `10094264`, so the existing 100GB pending job was left unchanged.
- ROB-27 preflight: from `eval/` in `/mnt/parscratch/users/acp21rjf/conda/main`, imported `eval_manager`/`run`, passed manager config checks, confirmed 15 models and 75 model/dataset evaluations, and verified configured dataset splits resolve to records (`tedlium` 11, `rev16` 16, `earnings22_full` 125, `this_american_life` 36, `earnings22` 6).
- ROB-27 queue probe: fresh 20h H100-NVL test-only submission estimated `2026-05-13T01:57:50`; 20h standard H100 test-only submission estimated `2026-05-04T15:30:35`. Existing job `10094264` still had earlier estimate `2026-05-03T05:20:00`, so it was kept.
- ROB-27 queue update: lowered pending full-eval job `10094264` walltime from 48h to the repo H100 launcher default of 20h using `scontrol update JobId=10094264 TimeLimit=20:00:00`; estimate remained `2026-05-03T05:20:00`.
- ROB-27 queue probe: 80GB/20h, 60GB/20h, and 80GB/12h H100-NVL test-only submissions all estimated `2026-05-04T02:57:50`; `hp-h100-nvl` rejected the account/partition combination. Existing job `10094264` remained the earliest viable full eval.
- ROB-27 queue update: `squeue --start` briefly lost the reservation for job `10094264` (`StartTime=Unknown`). Fresh test-only probes showed H100-NVL jobs pushed to `2026-05-12T18:57:50`, standard H100 at `2026-05-04T15:30:35`, and combined `gpu-h100,gpu-h100-nvl` accepted. Updated existing job with `scontrol update JobId=10094264 Partition=gpu-h100,gpu-h100-nvl`; Slurm restored an estimate of `2026-05-03T05:20:00`.

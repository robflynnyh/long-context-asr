...
- ROB-90 on branch `symphony/ROB-90-streaming-decoder-tedlium-eval`: added `StreamingDecoderASR.transcribe(...)` for decoder-only streaming inference, updated greedy two-head inference to score text by `P(not silence) * P(token)`, and kept the widely used generic `eval/run.py` unchanged after review feedback. The TEDLIUM runner now has a bounded utterance-output path for this sanity check; the ROB-76 two-head checkpoint `/store/store5/data/acp21rjf/spotify/streaming_decoder_asr_100m_two_head_mimas_full_epoch_rob76-mimas-3epoch-b48-two-head-20260517T140145Z/step_82737.pt` ran on Mimas GPU 2 with JSONL artifacts under `/store/store5/data/acp21rjf/symphony-job-artifacts/ROB-90/tedlium-run-rework-8utt/`; compact report committed at `symphony/reports/ROB-90-streaming-decoder-tedlium-eval.md`.

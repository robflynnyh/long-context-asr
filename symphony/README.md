...
- `scripts/linear_job_callback.py`: reusable stdlib-only Linear callback core
  for bounded job handoff comments and state transitions.
- `scripts/linear_stanage_callback.py` and `scripts/linear_mimas_callback.py`:
  thin target-specific callback entry points for Slurm/finalizer and detached
  `screen` wrappers.

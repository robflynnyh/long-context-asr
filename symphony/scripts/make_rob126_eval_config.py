...
def main():
    ...
    for run in manifest["runs"]:
        ...
        models.append(
            {
                "name": f"rob126_{run['label']}_top{run['unfreeze_top_n_layers']}_ctc_probe",
                "label": run["label"],
                "base_label": run.get("base_label", run["label"]),
                "learning_rate": run.get("learning_rate"),
                "encoder_lr_scale": run.get("encoder_lr_scale"),
                "probe_head": run.get("probe_head", "bilstm"),
                "hidden_state_weighted_sum": run.get("hidden_state_weighted_sum", True),
                "num_hidden_states": run.get("num_hidden_states"),
                "unfreeze_top_n_layers": run.get("unfreeze_top_n_layers"),
                "trainable_encoder_layers": ",".join(map(str, run.get("trainable_encoder_layers", []))),
                ...
                "backup_reason": run.get("backup_reason", ""),
                "path": str(ckpt),

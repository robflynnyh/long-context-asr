...
def base_config(args, head_name, checkpoint_label, checkpoint_file, learning_rate, run_label):
    checkpoint_path = Path(args.checkpoint_cache) / checkpoint_file
    source_checkpoint = f"{args.source_checkpoint_root.rstrip('/')}/{checkpoint_file}"
    ...
    return {
        ...
        "description": (
            "ROB-130 fully frozen ROB-70 low-mask BEST-RQ CTC probe using corrected "
        ...
        "wandb": {
            ...
            "project_name": "rob130_lowmask_corrected_probe",
...
def main():
    ...
    parser.add_argument("--checkpoint-cache", default="/store/store5/data/acp21rjf/symphony-job-artifacts/ROB-91/source-checkpoints")
    parser.add_argument("--source-checkpoint-root", default="/mnt/parscratch/users/acp21rjf/spotify/bestrq_ssl/6l_2048_1epoch_lr3e4_20260516")
    parser.add_argument("--checkpoint-labels", default="100pct")
    ...
    checkpoint_labels = parse_csv(args.checkpoint_labels) or ["100pct"]
    unknown_checkpoints = [label for label in checkpoint_labels if label not in CHECKPOINTS]
    if unknown_checkpoints:
        raise SystemExit(f"unknown checkpoint label(s): {', '.join(unknown_checkpoints)}")
    for label in checkpoint_labels:
        checkpoint_path = Path(args.checkpoint_cache) / CHECKPOINTS[label]
        if not checkpoint_path.is_file():
            raise SystemExit(f"missing local ROB-70 checkpoint for {label}: {checkpoint_path}")
    ...
    runs = []
    single_checkpoint = len(checkpoint_labels) == 1
    for checkpoint_label in checkpoint_labels:
        checkpoint_file = CHECKPOINTS[checkpoint_label]
        for head_name in heads:
            for learning_rate in learning_rates:
                run_label_parts = []
                if not single_checkpoint:
                    run_label_parts.append(checkpoint_label)
                run_label_parts.append(head_name)
                if not single_lr:
                    run_label_parts.append(f"lr{lr_token(learning_rate)}")
                run_label = "_".join(run_label_parts)
                config = OmegaConf.create(base_config(args, head_name, checkpoint_label, checkpoint_file, learning_rate, run_label))
                config_path = config_dir / f"rob130_{run_label}_frozen_weighted_ctc_probe.yaml"
                ...
                runs.append(
                    {
                        ...
                        "source_checkpoint": config.probe.source_checkpoint,
                        "local_checkpoint": config.probe.ssl_checkpoint,
    ...
    manifest = {
        ...
        "batch_size": args.smoke_batch_size if args.smoke else args.batch_size,
        "comparison_note": (
            "TEDLIUM transfer evidence only. This reruns the fully frozen ROB-70 "
            "low-mask weighted hidden-state probe on corrected STM utterance-boundary labels."
        ),
        "runs": runs,
    }

...
DECODER_STATE_MODE="${ROB192_TED_CHUNK_DECODER_STATE_MODE:-reset}"
...
on_exit() {
  ...
  set +e
  {
    ...
    echo "decoder_state_mode=${DECODER_STATE_MODE}"
  ...
  if [[ "${ROB192_TED_CHUNK_ENABLE_CALLBACK:-1}" == "1" ]]; then
    ...
    callback_args=(
      ...
      --metadata "decoder_state_mode=${DECODER_STATE_MODE}"
...
args=(
  ...
  --subsampling-mode "$SUBSAMPLING_MODE"
  --decode-mode "$DECODE_MODE"
  --decoder-state-mode "$DECODER_STATE_MODE"

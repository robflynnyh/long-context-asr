...
SUBGROUP_SHUFFLE_SIZE="${ROB209_SUBGROUP_SHUFFLE_SIZE:-0}"
if [[ "$SUBGROUP_SHUFFLE_SIZE" -le 0 ]]; then
  SUBGROUP_SHUFFLE_SIZE="$BATCH_SIZE"
fi
...
on_exit() {
  ...
  set +e
  {
    ...
    echo "subgroup_shuffle_size=${SUBGROUP_SHUFFLE_SIZE}"
...
prep_args=(
  ...
  --subgroup-shuffle-size "$SUBGROUP_SHUFFLE_SIZE"

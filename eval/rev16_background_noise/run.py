...
BACKGROUND_PATH = '/mnt/parscratch/users/acp21rjf/ElevatorMusic.wav'
...
def processing_chain(
    ...
    transform = AddBackgroundNoise(
        sounds_path = BACKGROUND_PATH,
        min_snr_db = min_snr_db,  
        max_snr_db = max_snr_db, 
        p = p
    )
    augmented_waveform = transform(waveform.squeeze(), sample_rate=16000)
    augmented_waveform = torch.from_numpy(augmented_waveform)[None]

    torchaudio.save('augmented.wav', augmented_waveform, 16000)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--min_snr_db', type=float, default=-0.0, help='min snr for augmentation')
    parser.add_argument('--max_snr_db', type=float, default=0.0, help='max snr for augmentation')

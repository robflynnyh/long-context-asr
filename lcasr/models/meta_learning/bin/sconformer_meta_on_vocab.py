...
class metadecoder(nn.Module):
    def __init__(self, d_model, vocab_size, norm=nn.LayerNorm, **kwargs):
        ...
        self.ff = nn.Linear(d_model, vocab_size) # ConformerFeedForward(d_model)
...
class SCConformerMeta(BaseModel): 
    def __init__(
        ...
        **kwargs
        ...
        self.meta_decoder = metadecoder(d_model, vocab_size +1, **kwargs)
        self.meta_vocab_ds = nn.Linear(vocab_size + 1, d_model)
        ...
        for param in self.meta_vocab_ds.parameters():
            param.requires_grad = True
    ...
    def forward(
        ...
        if not self.training: iterations = 5
        ...
        for i in range(iterations):
            ...
            audio_signal = decoder(x = decoder.norm(audio_signal) if self.legasee_double_norm else audio_signal, logits = True)

            self.reprs = audio_signal.clone()
            audio_signal = self.reprs

            audio_signal = self.meta_vocab_ds(audio_signal)
        ...
        final_posts = self.reprs - meta_pred   

        final_posts = final_posts.log_softmax(dim=-1) if not return_logits else final_posts

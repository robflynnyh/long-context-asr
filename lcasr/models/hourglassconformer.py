...
Conv1DSubsampling =  subsampling.Conv1DSubsampling
...
class ASRUpsampleDecoder(nn.Module):
    def __init__(
            self, 
            d_model, 
            vocab_size, 
        ...
        self.ff = nn.Linear(d_model, self.num_classes * upsampling_factor)
        self.upsampling_factor = upsampling_factor
        self.norm = norm_fn(d_model) if norm else nn.Identity()

    def forward(self, x, length=None, logits=False):
        x_norm = self.norm(x)
        
        x = self.ff(x_norm)
        
        x = rearrange(x, 'b n (c f) -> b (n f) c', f = self.upsampling_factor)
        if exists(length): length = length * self.upsampling_factor
        
        x = F.log_softmax(x, dim=-1) if not logits else x
        return x, length  

def subsampling_batchrenorm(d_model):
    return nn.Sequential(
        Rearrange('b c t -> b t c'),
        BatchRenorm1d(d_model),
        Rearrange('b t c -> b c t')
    )

class HourGlassConformer(BaseModel): 
    def __init__(
        self,
        vocab_size = 128,
        feat_in = 80,
        subsampling = 'dw_striding',
        subsampling_factor = 32,
        upsampling_factor = 4,
        ...
        **kwargs
        ...
        self.decoder = ASRUpsampleDecoder(
            d_model = d_model,
        ...
        # subsampling_args = {'subsampling_factor': self.subsampling_factor, 'feat_in': self.feat_in, 'feat_out': self.d_model, 'norm_out': True, 'default_norm':subsampling_batchrenorm}
        # self.subsampling = \
    ...
    def forward(
        ...
        final_posts, length = decoder(x = decoder.norm(audio_signal), length = length, logits = return_logits) # having decoder.norm should have been removed is sortof a bug but probably doesn't matter
...
if __name__ == '__main__':
    # run test
    model = HourGlassConformer(vocab_size=4096, head_dim=256, n_heads=3, attention_window_size=128)

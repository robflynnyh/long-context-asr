...
#             swiglu(self.in_dim, exp_f=1, out_dim=self.in_dim),
...
#         x_p = x_p / x_p.norm(dim=-1, keepdim=True)
#         csim = (torch.einsum('bnd,bnd->bn', x, x_p) * 2) + 2
#         if mask is not None: csim = csim.masked_fill(mask, 0)
#         return csim.sum() / x.shape[0]
...
class LearntLoss(nn.Module):
    def __init__(self, d_model):
        ...
        self.in_dim = d_model // 4
        self.shared_nn = nn.Sequential(
            swiglu(d_model, exp_f=1, out_dim=self.in_dim),
        )
        self.predictor = nn.Sequential(
            swiglu(self.in_dim, exp_f=1, out_dim=self.in_dim),
            swiglu(self.in_dim, exp_f=1),
...
class MinMaxEMATracker(nn.Module):
    def __init__(self, alpha=0.8): #99
        super().__init__()
        self.alpha = alpha
...
class MetaLayer(nn.Module):
    ...
    def forward(self, x, layer, mask=None, iterations=1):
        if layer == 0: self.w_delta = None
        assert iterations > 0
        x_norm = self.in_ln[layer](x)
        
        with torch.autograd.enable_grad(): # force grad to be enabled
            for _ in range(iterations):
                x_ff = self.in_ff(x_norm) if self.w_delta is None else torch.func.vmap(self.element_wise_fwd_w_delta, in_dims=0)(x_norm, self.w_delta)
                loss = self.learnt_loss(x_ff, mask)
                grad_x_ff = torch.autograd.grad(loss, x_ff, create_graph=True, retain_graph=True)[0]
                grad_ff = torch.einsum('bnq,bnz->bqz', grad_x_ff, x_norm)
                grad_ff = self.grad_clamp_vals[layer](grad_ff)
                print(grad_ff.min().item(), grad_ff.max().item(), loss)
...
#         self.in_ln = nn.ModuleList([norm_in_fn(d_model) for _ in range(layers)])
#         self.in_ff = nn.Linear(d_model, d_model)
#         self.learnt_loss = nn.ModuleList([LearntLoss(d_model) for _ in range(layers)])
#         #self.grad_clamp_vals = nn.ModuleList([MinMaxEMATracker() for _ in range(layers)])
#         self.out_swiglu = nn.ModuleList([
#             nn.Sequential(
#                 glu_norn(d_model),
#                 swiglu(d_model, exp_f=1),
#             ) 
...
#             for _ in range(iterations):
#                 x_ff = self.in_ff(x_norm) if self.w_delta is None else torch.func.vmap(self.element_wise_fwd_w_delta, in_dims=0)(x_norm, self.w_delta)
#                 loss = self.learnt_loss[layer](x_ff, mask)
#                 grad_x_ff = torch.autograd.grad(loss, x_ff, create_graph=True, retain_graph=True)[0]
#                 grad_ff = torch.einsum('bnq,bnz->bqz', grad_x_ff, x_norm)
#                 #print(grad_ff.min().item(), grad_ff.max().item())
#                 grad_ff = grad_ff.clamp(-0.3, 0.3) 
#                 self.w_delta = grad_ff * self.lr[layer].abs() if self.w_delta is None else self.w_delta + (grad_ff * self.lr[layer].abs())

#         x_ff_u = torch.func.vmap(self.element_wise_fwd_w_delta, in_dims=0)(x_norm, self.w_delta)
#         x_out = self.out_swiglu[layer](x_ff_u)
#         return x_out

...
class SCConformerMeta(BaseModel): 
    def __init__(
        ...
        **kwargs
        ...
        self.inference_lr = kwargs.get('inference_lr', 0.01)
    ...
    def forward(
        ...
        self.reprs = []
        self.grad_preds = []
        self.reprs.append(audio_signal)
        if self.training: self.reprs[-1].retain_grad()
        audio_signal = self.reprs[-1]

        meta_signal = audio_signal.clone()

        for lth, (layer, meta_layer) in enumerate(zip(self.layers, self.meta_layers)):
            ...
            if self.checkpoint_every_n_layers > 0 and lth % self.checkpoint_every_n_layers == 0:
                meta_signal = checkpoint(
                    self.create_custom_forward(meta_layer), 
                    meta_signal, # x
                    att_mask, # att_mask
                    length,
                    pad_mask, # pad_mask
        ...
        self.grad_preds.append(self.meta_decoder(meta_signal))

...
class MetaConformer(nn.Module): 
    def __init__(
        ...
        **kwargs
        ...
        self.decoder = MetaASRLinearSCDecoder(
            d_model = d_model,
            layers = n_layers,
    ...
    def forward_for_export( # jus change this to forward..
        self, 
        audio_signal, 
        decoder, 
        length = None, 
        ...
        assert return_logits == False, 'lazy rn do later'
        max_audio_length: int = audio_signal.size(-1)
        cached_kvs, cached_kv_lengths = None, None # need to remove, lazy rn
        ...
        for lth, layer in enumerate(self.layers):
            ...
            if lth != len(self.layers) - 1 and self.self_conditioning:
                iterim_post = torch.nn.functional.softmax(decoder(x_all=audio_signal, logits=True, layer=lth, mask=mask), dim=-1)
        ...
        final_posts = decoder(x_all = audio_signal, logits = return_logits, layer = self.n_layers - 1, mask=mask)
...
class swiglu(nn.Module):
    def __init__(self, d_model, exp_f=1, out_dim=None):
        super().__init__()
        self.d_model = d_model
        self.out_dim = out_dim if out_dim is not None else d_model
        self.in_ff = nn.Linear(d_model, d_model*exp_f*2)
        self.out_ff = nn.Linear(d_model*exp_f, self.out_dim)
        self.silu = nn.SiLU()

    def forward(self, x):
        a, b = self.in_ff(x).chunk(2, dim=-1)
        return self.out_ff(a * self.silu(b))


class GoldAugComparison(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.in_dim = d_model // 4
        self.shared_nn = nn.Sequential(
            nn.LayerNorm(d_model),
            nn.Linear(d_model, self.in_dim),
            nn.SiLU(),
            nn.Linear(self.in_dim, self.in_dim),
        )
        self.predictor = nn.Sequential(
            swiglu(self.in_dim*2, exp_f=1, out_dim=self.in_dim),
            swiglu(self.in_dim, exp_f=1),
        )

    def forward(self, x, n_gold, n_aug, mask=None):
        x = self.shared_nn(x)
        x_gold, x_aug = torch.split(x, [n_gold, n_aug], dim=0)
        x_gold = x_gold.repeat((n_aug // n_gold, 1, 1))
        x_all = torch.cat([x_gold, x_aug], dim=-1)
        x_aug = self.predictor(x_all)
        x_gold = x_gold / x_gold.norm(dim=-1, keepdim=True)
        x_aug = x_aug / x_aug.norm(dim=-1, keepdim=True)
        csim = (torch.einsum('bnd,bnd->bn', x_gold, x_aug) * 2) + 2
        if mask is not None: csim = csim.masked_fill(mask[:n_gold], 0)
        return csim.sum() / n_gold


class MetaASRLinearSCDecoder(nn.Module):
    def __init__(self, d_model, vocab_size, layers, norm=False, norm_fn=DEFAULT_NORM, base_lr=1e-10):
        super().__init__()
        # Add 1 for blank char
        self.layers = layers
        self.num_classes = vocab_size + 1
        self.ff = nn.Linear(d_model, self.num_classes)
        self.in_ff = nn.Linear(d_model, d_model)
        ...
        self.w_delta = None
        self.gold_aug_losses = nn.ModuleList([GoldAugComparison(d_model) for _ in range(layers)])

        self.inner_ff = nn.Sequential(
            nn.LayerNorm(d_model),
            nn.SiLU(),
            nn.Linear(d_model, d_model),
        )
        self.norm_out = norm_fn(d_model) 
 
    def element_wise_fwd_w_delta(self, x, w_delta):
    ...
    def forward(self, x_all, layer, logits=False, augmented_samples=1, mask=None):
        assert augmented_samples > 0, ""
        if layer == 0: self.w_delta = None
        if layer == self.layers-1: print(self.lr.abs())
        gold_aug_loss = self.gold_aug_losses[layer]
       
        B, N, D = x_all.shape
        ptn1, ptn2 = '(b s) n d -> b s n d', 'b s n d -> (b s) n d'

        augment_loop = lambda a: torch.func.vmap(self.element_wise_fwd_w_delta, in_dims=0)(a, self.w_delta)
        delta_ff = lambda a: rearrange(torch.func.vmap(augment_loop, in_dims=0)(rearrange(a, ptn1, s=n_gold)), ptn2)

        p_aug = augmented_samples / (augmented_samples + 1)
        p_gold = 1 - p_aug
        n_aug, n_gold = int(B*p_aug), int(B*p_gold)
        
        x_norm = self.norm(x_all)

        x = self.in_ff(x_norm) if self.w_delta is None else delta_ff(x_norm)
        aux_loss = gold_aug_loss(x, n_gold, n_aug, mask=mask)
        
        grad_x = torch.autograd.grad(outputs=aux_loss, inputs=x, retain_graph=True, create_graph=True)[0]
        grad_ff = rearrange(torch.einsum('bnq,bnz->bqz', grad_x, x_norm), '(b s) n d -> b s n d', s=n_gold).sum(0)
        grad_ff = grad_ff #* 0 #.clamp(-0.00000003, -0.00000003)
        print(aux_loss, layer, grad_ff.max().item(), grad_ff.min().item())
        self.w_delta = grad_ff * self.lr[layer].abs() if self.w_delta is None else self.w_delta + grad_ff * self.lr[layer].abs()

        x_out = self.inner_ff(delta_ff(x_norm))
        x = self.norm_out(x_out) + x
        x = self.ff(x)

...
class MetaLayer(nn.Module):
    def __init__(
        ...
        #self.deltas = [None]*layers

    def element_wise_fwd_w_delta(self, x, grad_x_ff): # used with vmap
        w_delta = torch.einsum('nq,nz->qz', grad_x_ff, x)
        w_delta = self.clip_norm(w_delta)
        #self.deltas[self.layer] = w_delta if self.deltas[self.layer] is None else self.deltas[self.layer] * 0.1 + w_delta * 0.9
    ...
    def forward(self, x, layer, mask=None):
        ...
        with torch.autograd.enable_grad(): 
            ...
            with torch.enable_grad():
                x_ff_q, i, l = self.vqs[layer](x_ff)

            args = (x_ff, x_ff_q, mask) if mask is not None else (x_ff, x_ff_q)
            batched_grad_x_ff = torch.func.vmap(torch.func.grad(self.learnt_loss), in_dims=0)(*args)
        
        x_ff_u, grad_ff = torch.func.vmap(self.element_wise_fwd_w_delta, in_dims=0)(x_norm, batched_grad_x_ff)

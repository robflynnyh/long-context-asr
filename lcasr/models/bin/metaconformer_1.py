...
class LearntLoss(nn.Module):
    def __init__(self, d_model):
        ...
        self.a_ff = ConformerFeedForward(self.in_dim)
        self.b_ff = ConformerFeedForward(self.in_dim)
    ...
    def forward(self, x, mask=None):
        ...
        csim = (torch.einsum('bnd,bnd->bn', a, b) * 2) + 2
...
class MetaLayer(nn.Module):
    ...
    def forward(self, x, layer, mask=None):
        ...
        print(grad_ff.min().item(), grad_ff.max().item(), loss, self.lr[layer].item())

...
class metadecoder(nn.Module):
    def __init__(self, d_model, norm=LayerNorm):
        ...
        self.ff2 = nn.Linear(d_model, d_model) # ConformerFeedForward(d_model)
        self.embeddings = nn.Parameter(torch.randn(1, d_model, d_model))
        nn.init.normal_(self.embeddings, mean=0, std=0.05)
        self.proj = ConformerFeedForward(d_model)

    def forward(self, x): 
        x = self.norm(x)
        a = self.ff1(x)
        a = a.softmax(dim=-2)
        b = (a * x).sum(dim=-2, keepdim=True)
        b = self.ff2(b)
        b = b.repeat(1, x.shape[-1], 1)
        b = b + self.embeddings
        c = self.proj(b).mean(0)
        return c
...
class SCConformerMeta(BaseModel): 
    def __init__(
        ...
        **kwargs
        ...
        self.meta_proj = PreNorm(d_model, nn.Linear(d_model, d_model, bias=False), norm = default_norm)
        ...
        for param in self.meta_proj.parameters():
            param.requires_grad = True
    ...
    def forward(
        ...
        iterations = 1
        if not was_training: iterations = 1
        ...
        for i in range(iterations):
            ...
            #if was_training or i ==i: #< iterations - 1:
            if was_training or i < iterations - 1:
                ...
                if not was_training: 
                    grad_pred = meta_pred / meta_pred.norm(dim=-1, keepdim=True, p=2)

                    all_params = [self.initial_signal] #+ params
                    all_param_grads = torch.autograd.grad(outputs=self.reprs, inputs=all_params, grad_outputs=grad_pred, retain_graph=False)
        ...
        if was_training and self.rotary_pos_emb is not None:

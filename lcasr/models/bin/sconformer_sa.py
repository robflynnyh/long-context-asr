...
class SpeakerCountingTokens(nn.Module):
    def __init__(self, d_model, n_tokens):
        super().__init__()
        self.tokens = nn.Parameter(torch.randn(n_tokens, d_model))
        nn.init.normal_(self.tokens) 

    def forward(self, x):
        tokens = self.tokens[None].expand(x.size(0), -1, -1)
        return torch.cat((tokens, x), dim=1) # cat to start of sequence


class SCConformerSA(BaseModel): 
    def __init__(
        self,
        vocab_size = 128,
        feat_in = 80,
        ...
        **kwargs
        ...
        self.blacklist_weight_decay_modules = (nn.Linear, ConformerFeedForward, nn.Conv1d, nn.Conv2d, RotaryPositionalEmbedding, SpeakerCountingTokens)

        self.decoder_norm = decoder_norm

        self.use_rotary = use_rotary

        self.num_speaker_counting_tokens = 10
        self.speaker_counting_tokens = SpeakerCountingTokens(d_model, n_tokens = self.num_speaker_counting_tokens)
    ...
    def forward(
        ...
        audio_signal = self.speaker_counting_tokens(audio_signal)
        print(self.num_speaker_counting_tokens)
        length += self.num_speaker_counting_tokens

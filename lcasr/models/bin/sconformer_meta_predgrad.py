...
class SCConformerMeta(BaseModel): 
    def __init__(
        ...
        **kwargs
        ...
        self.meta_residual_proj = PreNorm(d_model, nn.Linear(d_model, d_model), norm = default_norm)
    ...
    def forward(
        ...
        if self.training:
            ...
            for param in self.meta_layers.parameters():
                param.requires_grad = True
            for param in self.meta_residual_proj.parameters():
                param.requires_grad = True
        else:
            for param in self.parameters():
                param.requires_grad = True
        ...
        for i in range(iterations):
            ...
            audio_signal = audio_signal + self.meta_residual_proj(meta_signal)
            ...
            if i < iterations - 1:
                if not was_training: 
                    grad_pred = meta_pred
                    param_inputs = [p for p in model.meta_layers.parameters()]
                    all_param_grads = torch.autograd.grad(outputs=self.reprs, inputs=param_inputs, grad_outputs=grad_pred, retain_graph=False)
                    
                    for param, grad in zip(param_inputs, all_param_grads):
                        new_weight = param.data - grad * 1e-5
                        param.data = new_weight
            # else:
            #     if not was_training:
            #         x_in = self.static_initial_signal.detach()
            #         x_in.requires_grad = True
            #         x_out = self.main_layers(x_in, att_mask, length, pad_mask, rotary_emb_fn)
            #         delta = (x_out - audio_signal)
            #         params = [p for p in self.layers[0].parameters()]
            #         all_param_grads = torch.autograd.grad(outputs=x_out, inputs=params, grad_outputs=delta, retain_graph=False)
            #         for param, grad, ema_module in zip(params, all_param_grads, self.emas):
            #             new_weight = ema_module(param.data - grad * 1e-5)
            #             #print(new_weight)
            #             param.data = new_weight
                    #audio_signal = x_out
